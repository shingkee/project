<?php
class Membership_Gallery_Model_GalleryImageAttachment extends Membership_Base_Model_Base {
	
	public function addImageAttachment($image, $galleryId) {
		...
		$savePath = $this->generateFilePath('/membership/attachments/', 'jpg', $galleryId, $userId);
		...
		$query = $this->preparePrefix("
			INSERT INTO `{prefix}photo_gallery_images`
				(`photo_gallery_id`, `height`, `width`, `url`)
			VALUES ('%d', '%d', '%d', '%s');
			");

		$this->db->query(
			$this->db->prepare($query, array($galleryId, $size['height'], $size['width'], $source))
		);
	...
	public function deleteImagesFromGallery($galleryId) {
		
		if($galleryId) {
			$query = $this->preparePrefix("
				SELECT `id`, `url` FROM `{prefix}photo_gallery_images`
				WHERE `photo_gallery_id` = '%d'
			");
	...
	public function removeImageById($id, $imageUrl = null) {
		...
		$queryRemove = $this->preparePrefix("DELETE FROM `{prefix}photo_gallery_images` WHERE `id` = '%d'");
	...
	public function generateFilePath($directory, $ext, $galleryId = '', $urserId = '') {
		$uniqid = preg_replace(
			array('`ad`', '`\.`'),	// Avoid add block false positives
			array('a0', ''),
			uniqid(null, true)
		);
		$fileName = $urserId . '_' . $uniqid . '_' . $galleryId . '.' . $ext;
	...
	public function setImagesPositions(array $imagesArray) {
		if(count($imagesArray)) {
			...
			$queryString = "UPDATE `{prefix}photo_gallery_images` SET `position`= CASE " . $queryString;
