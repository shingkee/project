...
class EPKB_KB_Config_Overview {
	...
	public static function display_overview( $kb_config, $feature_specs, $form ) {
		...
		foreach( $kb_main_pages_info as $post_id => $post_info ) {
			...
			$kb_main_pages_url .= '<a href="' . ( empty($main_page_view_url) || is_wp_error( $main_page_view_url ) ? '' : $main_page_view_url ) . '" target="_blank">View</a> ';
			$post_link = get_edit_post_link( $post_id );
			$kb_main_pages_url .= ' &nbsp;&nbsp;<a href="' . ( empty($post_link) ? '' : $post_link ) . '" target="_blank">' . esc_html__( 'Edit', 'echo-knowledge-base' ) . '</a></li>';
		}

		$kb_main_pages_url = empty($kb_main_pages_url) ? ' None found' : $kb_main_pages_url;
		    ...
		    <section class="overview-info-section">
	            <div class="overview-error">
	                <div class="overview-header">
	                    <div class="overview-title">Errors</div>
	                    <div class="overview-brief"><span class="overview-icon ep_font_icon_error_circle"></span> Critical Errors Detected</div>
	                </div>
	                <div class="overview-content">		            <?php
						foreach( $add_on_messages as $add_on_name => $add_on_message ) {
							...
							echo
									...
									'<h4>' . esc_html($add_on_name) . ': License issue</h4>' .
		...
		if ( $is_status_error ) {
			...
			$brief = '<div class="overview-brief"><span class="overview-icon ep_font_icon_question"></span> Potential issues.</div>
	                  <div class="overview-toggle">View Details</div>';
		} else {
			$class_status = 'overview-success';
			$brief = '<div class="overview-brief"><span class="overview-icon ep_font_icon_checkmark"></span> All Good, No Issues.</div>';
        }        ?>

        <section class="overview-info-section">
            <div class="<?php echo $class_status; ?>">
                <div class="overview-header">
                <div class="overview-title">Warnings</div>
                    <?php echo $brief; ?>
            </div>
                <div class="overview-content">				<?php
					if ( $is_status_error ) {
        ...
        <section class="overview-info-section">
            <div class="overview-info">
                <div class="overview-header">
                    <div class="overview-title">Statistics</div>
                    <div class="overview-brief"><span class="overview-icon ep_font_icon_data_report"></span>Knowledge Base Statistics</div>
                    <div class="overview-toggle">View Details</div>
                </div>
                <div class="overview-content">
	                ...
	                <div class="overview-info-widget">
		                <div class="widget-header">Articles</div>
		                <div class="widget-content">
        ...
        <section class="overview-info-section">
            <div class="overview-config">
                <div class="overview-header">
                    <div class="overview-title">Configuration</div>
                    <div class="overview-brief"><span class="overview-icon ep_font_icon_gear"></span>Additional Global Settings</div>
                    <div class="overview-toggle">View Details</div>
                </div>
                <div class="overview-content">
	...
	public static function get_kb_status_line( $kb_config, $chosen_layout='' ) {
		...
		$output = '<div id="status_line" class="kb_status ' . $status_class . '"><strong>';
		$output .= 'KB Status: ' . esc_html__( $status, 'echo-knowledge-base' ) . '</strong>';
