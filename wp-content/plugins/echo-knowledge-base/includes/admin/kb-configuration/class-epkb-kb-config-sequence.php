...
class EPKB_KB_Config_Sequence {
	...
	public function update_articles_sequence( $kb_id, & $new_kb_config ) {
		...
		if ( $is_custom_sequence ) {
			...
			if ( $new_sequence === false ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Could not save article sequence' . ' (B31)', 'echo-knowledge-base' ));
			}

			$orig_articles_obj = $this->update_articles_order( $kb_id, $new_sequence );
			if ( $orig_articles_obj === false ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Could not save article sequence' . ' (B32)', 'echo-knowledge-base' ));
			}

			$result = EPKB_Utilities::save_kb_option( $kb_id, EPKB_Articles_Admin::KB_ARTICLES_SEQ_META, $orig_articles_obj->ids_array, true );
			if ( is_wp_error( $result ) ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Could not save article sequence' . ' (B33)', 'echo-knowledge-base' ));
			}
		} else {
	...
	public function update_categories_sequence( $kb_id, & $new_kb_config ) {
		...
		if ( $is_custom_sequence ) {
			...
			$new_sequence = $this->get_new_sequence();
			if ( $new_sequence === false ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Could not save category sequence' . ' (B22)', 'echo-knowledge-base' ));
			}
			...
			if ( $orig_categories_obj === false ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Could not save category sequence' . ' (B24)', 'echo-knowledge-base' ));
			}
			...
			if ( is_wp_error( $result ) ) {
				EPKB_Utilities::ajax_show_error_die(__( 'Could not save category sequence' . ' (B23)', 'echo-knowledge-base' ));
			}
		} else {
	...
	public function get_new_sequence() {
		...
		$new_sequence = isset($_REQUEST['epkb_new_sequence']) ? $this->sanitize_sequence( $_REQUEST['epkb_new_sequence'] ) : false;
		if ( $new_sequence === false ) {
			return false;
		}
		...
		$top_cat_sequence = isset($_REQUEST['top_cat_sequence']) ? $this->sanitize_top_cat_sequence( $_REQUEST['top_cat_sequence'] ) : false;
