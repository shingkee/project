...
class EPKB_KB_Config_DB {
	...
	public function get_kb_ids() {
		...
		if ( empty($kb_ids) || ! isset($kb_ids[EPKB_KB_Config_DB::DEFAULT_KB_ID]) ) {
			EPKB_Logging::add_log("Ensure we have at least default KB ID.");
	...
	public function get_kb_config( $kb_id ) {
		...
		if ( empty($config) || ! is_array($config) ) {
			EPKB_Logging::add_log("Did not find KB configuration (DB23).", $kb_id);
			return new WP_Error('DB23', "Did not find KB configuration");
		}
