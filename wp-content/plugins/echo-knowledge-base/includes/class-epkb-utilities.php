...
class EPKB_Utilities {
	...
	public static function get_html_message_box( $message, $title='', $type='success' ) {
		...
		return
			"<div class='eckb-top-notice-message'>
				<div class='contents'>
					<span class='$type'>
						$title
	...
	public static function get_variable_string( $var ) {
		...
		foreach ($var as $key => $value) {
			...
			if ( ! is_array($value) ) {
				$output .= "[" . $key . " => " . self::get_variable_not_array( $value ) . "]";
				continue;
			}
			...
			foreach($value as $key2 => $value2) {
				...
				if ( is_array($value2) ) {
					...
					$output .= "[" . $key2 . " => " . self::get_variable_not_array( $value2 ) . "]";
					continue;
				}
            }
		}
		...
		foreach( $category_seq_data as $category_id => $sub_categories ) {
			if ( ! empty($articles_seq_data[$category_id]) ) {
				...
				if ( ! empty($keys[2]) && EPKB_Utilities::is_positive_int( $keys[2] ) ) {
					return $return_post_id ? $keys[2] : EPKB_Utilities::get_kb_post_secure( $keys[2] );
				}
			}
