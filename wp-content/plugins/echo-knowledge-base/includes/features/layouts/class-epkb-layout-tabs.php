...
class EPKB_Layout_Tabs extends EPKB_Layout {
	...
	*/
	public function generate_kb_main_page() {
		...
		$active_tab = isset($_REQUEST['top-category']) ? sanitize_text_field( $_REQUEST['top-category'] ) : '';
		$active_cat_id = '0';
		$first_tab = true;
		...
		foreach ( $this->category_seq_data as $category_id => $box_categories_array ) {
			...
			<div <?php echo $class1; ?> > <?php
				...
				if ( empty($box_category_list) && ! empty($articles_coming_soon_msg) ) {
					echo '<div class="epkb-articles-coming-soon" ' . $top_category_id_data . ' data-kb-type=top-category-no-articles ' . '>' . esc_html__( $articles_coming_soon_msg, 'echo-knowledge-base' ) . '</div>';
				}
