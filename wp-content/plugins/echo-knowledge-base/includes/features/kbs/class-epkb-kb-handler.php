...
class EPKB_KB_Handler {
	...
	public static function add_new_knowledge_base( $new_kb_id, $new_kb_main_page_title, $new_kb_main_page_slug='' ) {
		...
		if ( is_wp_error( $kb_config ) || ! is_array($kb_config) ) {
			...
			EPKB_Logging::add_log("Found KB configuration when adding new KB", $new_kb_id, $kb_config);
		}
	...
	private static function create_sample_categories( $new_kb_id ) {
		...
		$sample_cat = self::create_sample_category_and_article( $new_kb_id, $kb_category_taxonomy_name, 'Articles', 'Knowledge Base Article', null );
		if ( empty($sample_cat) ) {
			return;
		}

		// second create sub-category with an article
		$sample_sub_cat = self::create_sample_category_and_article( $new_kb_id, $kb_category_taxonomy_name, 'Other', 'Knowledge Base Article #2', $sample_cat['category_id'] );
	...
	private static function create_sample_category_and_article( $new_kb_id, $kb_category_taxonomy_name, $category_name, $post_title, $parent_id ) {
		...
		$category_name = __( $category_name, 'echo-knowledge-base' );
		$post_title = __( $post_title, 'echo-knowledge-base' );
	...
	public static function update_existing_knowledge_base( $new_kb_id, $new_kb_main_page_title, $new_kb_main_page_slug='' ) {
		...
		if ( is_wp_error( $kb_config ) ) {
			EPKB_Logging::add_log("Could not retrieve KB configuration", $new_kb_id, $kb_config);
			return $kb_config;
		}
	...
	public static function get_current_kb_id() {
		...
		$epkb_post_type = empty($_REQUEST['post_type']) ? '' : preg_replace('/[^A-Za-z0-9 \-_]/', '', $_REQUEST['post_type']); // sanitize_text_field( $_REQUEST['post_type'] );
		if ( ! empty($epkb_post_type) && $epkb_post_type != 'page' ) {
			$kb_id = self::get_kb_id_from_post_type( $epkb_post_type );
		}
		...
		if ( is_wp_error( $kb_id ) && ! empty($epkb_action) && strpos( $epkb_action, self::KB_POST_TYPE_PREFIX ) !== false ) {
			$found_kb_id = EPKB_Utilities::sanitize_int( $epkb_action );
			if ( ! empty($found_kb_id) && EPKB_Utilities::is_positive_int( $found_kb_id ) ) {
				$kb_id = $found_kb_id;
			}
		}
